...
\newcommand{\tRange}{tRange}

\begin{equation}\label{eq:lxyPixel}
\begin{array}{c}
(\discreteR + \delR)  \cdot \uSrc = \discreteDst + \delDst\\
therefore\quad \delR  \cdot \uSrc = \delDst
\end{array}
\end{equation}

The continuous rotation \uR can be expressed as the sum of its discrete representation \dR and the perturbation \delR. The discrete rotation can be expressed as a rational fraction against the elements of \dR.

\begin{equation}\label{eq:dRRange}
\begin{array}{c}
\uR = \dR + \delR\\
...
It should be noted that --- unlike the source type \uSrc --- the perturbation can be negative. 

\begin{equation}\label{eq:allSrc}
\begin{array}{c}
\quad max(\delR \cdot \uSrc)~\forall~\uSrc\\
\left(
\begin{array}{ccc}
