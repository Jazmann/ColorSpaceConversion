...
\begin{aligned}
\lambda^+(\mu,\sigma)  &=  \xMin + \xRange \sqrt{2} \sigma  \text{erf}^{-1}\left((\text{dL}-1) \Sigma^- -\text{dL} \; \Sigma^+ \right)+\mu \\
\lambda^-(\mu,\sigma)  &=  \xMin + \xRange \sqrt{2} \sigma  \text{erf}^{-1}\left((\text{dL}-1) \Sigma^+-\text{dL} \; \Sigma^-\right)+\mu
\end{aligned}
\end{equation}
...
\Delta(\mu,\sigma) = \frac{\yRange}{\xRange}  \frac{ \sqrt{2} }{ \sqrt{\pi } \sigma  \left(\Sigma^+-\Sigma^-\right)}
\end{equation}
...
\min\left\{ \Delta(\mu,\sigma) , 1\right\} \srcRange L(\theta)\dstRange 
\end{equation}
...
We can now re-formulate the transformation matrix in three different ways: pure rotation without rescaling, unit scaling which preserves the discretization of the axis before the rotation and scaled which shortens the axis as much as possible without loosing statistically relavent data. 
...
The remaining two roations can be specified by application of a scaling to each row
