...
\Lambda^-(\mu,\sigma) = \xMin + \xRange \mu - \xRange \sigma  \sqrt{-2 \log \left(\sigma  \left(\Sigma^+-\Sigma^-\right)\right)+2 \log \left(\frac{\xRange}{\yRange}\right)+\log \left(\frac{2}{\pi }\right)}
\end{equation}
...
\lambda^+(\mu,\sigma)  =  \xMin + \xRange \sqrt{2} \sigma  \text{erf}^{-1}\left((-\text{dL}) \Sigma^+ +(\text{dL}-1) \; \Sigma^-\right)+\mu
\quad \text{where} \quad
