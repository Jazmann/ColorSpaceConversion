...
One further optimisation is possible if we consider the redistribution of the values which is performed after the rotation. Not all information contained in the source is of equal value and so an error introduced by using a less accurate discretization of the rotation may well be mitigated by the subsequent redistribution. Halving the discrete range will introduce errors of at most 1 for some of the pixel values. In general using $\tRange = 2^{1-n)} \dstRange$ will introduce a maximum error of $\frac{2^n -1}{\dstRange}$ into the resulting matrix We must now establish which elements are affected.
...
\begin{cases}
 ...
 -1 & 0\leq \theta <\frac{5 \pi }{6}\lor \frac{11 \pi }{6}<\theta <2 \pi  \\
\end{cases}
 & 
\begin{cases}
 1 & 0\leq \theta <\frac{\pi }{2}\lor \frac{3 \pi }{2}<\theta <2 \pi  \\
 -1 & \frac{\pi }{2}<\theta <\frac{3 \pi }{2} \\
\end{cases}
 & 
\begin{cases}
 1 & \frac{\pi }{6}<\theta <\frac{7 \pi }{6} \\
 -1 & 0\leq \theta <\frac{\pi }{6}\lor \frac{7 \pi }{6}<\theta <2 \pi  \\
\end{cases}
 \\
 
\begin{cases}
 1 & \frac{\pi }{3}<\theta <\frac{4 \pi }{3} \\
 -1 & 0\leq \theta <\frac{\pi }{3}\lor \frac{4 \pi }{3}<\theta <2 \pi  \\
\end{cases}
 & 
\begin{cases}
 1 & \pi <\theta <2 \pi  \\
...
\end{cases}
 & 
\begin{cases}
 1 & 0\leq \theta <\frac{2 \pi }{3}\lor \frac{5 \pi }{3}<\theta <2 \pi  \\
...
where 1 indicates that the element shares its sign with another element in the same row and -1 indicates the opposite.
