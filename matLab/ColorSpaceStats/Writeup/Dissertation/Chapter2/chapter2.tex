...
Once the fingertip has been identified, its features are obtained using the SURF algorithm method provided by OpenCV's SURF class; the static features are used to <something>, while the dynamic features (blood flow, finger deformation due to pressure, etc.) are used to measure pressure. Differences in pressure are then found by finding the deltas between the images.
...
0 & 1 & 0 & 0 & Color & XOR(Color, Region)\\\hline
1 & 0 & 1 & 0 & Color & XOR(Color, Region)\\\hline
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.70\textwidth]{Chapter2/Figs/2bitImage.eps}\label{fig:2BitImage}
    \caption{The improved contour method in action.}
\end{figure}
