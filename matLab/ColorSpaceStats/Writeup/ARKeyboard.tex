...
For the purposes of this project, a color space dedicated to human skin is necessary. When it comes to pattern recognition, it's beneficial to reduce the number of channels which the algorithm has to process, as well as reducing the information density in those channels. It is also hoped that, in an appropriate color space, it will be a simple matter to separate the characteristics associated with saturation of tissue with blood, which changes under mechanical stress from the unchanging pigmentation of the cells. ~\cite{Stamatas2004} Color spaces, when they're normally defined, are a combination of rotations, scaling and translations, while some (e.g. HSV~\cite{Vezhnevets2003}~\cite{Zarit1999a}) also perform a coordinate transform. This color space is unusual because the color space we defined uses a non-linear redistribution of the values. It should be noted that this is not a pixel-based classifier. ~\cite{Jones2002} However, seeing as the color space contains both the statistical information and the skin color information, it can be interpreted in a probabilistic way.

For the sake of comparison, the following four color spaces --- which are often used for skin detection --- have been evaluated for the purposes of this project:
