...
We need to consider the requested compression of information alongside the spread of information caused by the rotation, and the desired focus on the specific region of interest dictated by the statistics. Each axis in the color space is to be represented by a discrete set of numbers. The size of these sets dictates the discretization of the axis and the ratios between then indicates the spread or compression of the information they contain. We assume that the RGB axis are each discretized to the same extent each containing $\text{rgbRange}$ values. After the application of the un-normalized rotational transformation the axis contain differing numbers of values given by $\text{sRange}_1 =\text{rgbRange} \sqrt{3} $, $\text{sRange}_2 = \text{rgbRange} L_2(\theta) $  and $\text{sRange}_3 = \text{rgbRange} L_3(\theta)$.
...
The length of the axis after rescaling should be which ever is the shorter of 

\begin{equation}\label{eq:RescaleAxis}
\frac{\text{rgbRange} L(\theta)}{dRange} \Lambda(\mu,\sigma)
\end{equation}

The condition for rescaling the axis is:

\begin{equation}\label{eq:RescaleAxis}
\frac{dRange}{\text{rgbRange} L(\theta)} \Delta \le 1
\end{equation}

Where $dRange$ is the number of values which the destination type can contain, and $\text{rgbRange}$ is the number of values the RGB source type can contain.

\begin{equation}\label{eq:UnscaledAxis}
sRange = \text{rgbRange} L(\theta) \Delta > 1
\end{equation}
