function [ ] = getSIFT(imgCell,fig,chan)

kpts = cell(7);
kptsImg = cell(7);

kpts{1} = cv.SIFT(imgCell{1}(:,:,chan));
kpts{2} = cv.SIFT(imgCell{2}(:,:,chan));
kpts{3} = cv.SIFT(imgCell{3}(:,:,chan));
kpts{4} = cv.SIFT(imgCell{4}(:,:,chan));
kpts{5} = cv.SIFT(imgCell{5}(:,:,chan));
kpts{6} = cv.SIFT(imgCell{6}(:,:,chan));
kpts{7} = cv.SIFT(imgCell{7}(:,:,chan));
